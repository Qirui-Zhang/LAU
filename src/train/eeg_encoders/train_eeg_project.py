...
import re
import tqdm
from src.models.eeg_encoders.EEGProject.eeg_project_encoder import EEGProject_Encoder
from src.data.eeg_dataset import EEGDataset
from src.models.losses import LowLevelLoss
...
def train(config_path):
    ...
    # 初始化模型
    model = EEGProject_Encoder().to(device)
    ...
    # # 保存最后一个模型
    # ckpt_name = exp_cfg["checkpoint_format"].format(subject=subjects[0], epochs=epochs)
    # saving_path = os.path.join(save_dir, ckpt_name)
    # torch.save(model.state_dict(), saving_path)
    # print(f"Model saved at {saving_path}")
    # log += f"Model saved at {saving_path}\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default=f"configs/eeg_encoders/eeg_project_config.json", help="配置文件路径")
