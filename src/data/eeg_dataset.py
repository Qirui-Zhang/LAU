...
class EEGDataset():
    ...
    def __init__(self, data_path="./data/processed",
                 eeg_data_path=None, img_directory=None,
                 exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes=None,
                 pictures=None, val_size=None):
        self.data_path = data_path
        self.eeg_data_path = eeg_data_path \
            if eeg_data_path is not None \
            else os.path.join(data_path, "Preprocessed_data_250Hz")
        self.img_directory_training = os.path.join(img_directory, "training_images") \
            if img_directory is not None \
            else os.path.join(data_path, "image_set", "training_images")
        self.img_directory_test = os.path.join(img_directory, "test_images") \
            if img_directory is not None \
            else os.path.join(data_path, "image_set", "test_images")
        self.train = train
        self.subject_list = os.listdir(self.eeg_data_path)
    ...
    def load_data(self):
        ...
        for subject in self.subjects:
            if self.train:
                ...
                file_path = os.path.join(self.eeg_data_path, subject, file_name)
                data = np.load(file_path, allow_pickle=True)
                print("shape of data:" + str(data['preprocessed_eeg_data'].shape))
